...
class Stage2UnitTest : PhotoEditorUnitTest<MainActivity>(MainActivity::class.java) {
    ...
    companion object {
        const val MESSAGE_NULL_AFTER_SL_BRIGHTNESS = "Image was null after slBrightness triggered"
        const val MESSAGE_INTENT_NOT_FOUND = "No intent was found by tests. Have you launched an intent?"
        const val MESSAGE_WRONG_VALUES = "Wrong values after brightness applied."
        const val MARGIN_ERROR = 1
        const val CALCULATION_WAIT_TIME = 200L
    }
    ...
    fun test04_checkSliderNotCrashing() {
        testActivity {
            ...
            assertNotNull(MESSAGE_NULL_AFTER_SL_BRIGHTNESS, bitmap)
        }
    }
    ...
    fun test05_checkDefaultBitmapEdit() {
        testActivity {
            ...
            val actualImage1 = (ivPhoto.drawable as BitmapDrawable).bitmap ?: throw AssertionError(
                MESSAGE_NULL_AFTER_SL_BRIGHTNESS
            )
            val actualRgb1 = singleColor(actualImage1)
            assertColorsValues(
                "$MESSAGE_WRONG_VALUES For x=70, y=60",
                expectedRgb1,
                actualRgb1,
                MARGIN_ERROR
            )
    ...
    fun test06_checkLoadedBitmapEdit() {
        testActivity {
            ...
            val actualIntent = shadowActivity.peekNextStartedActivityForResult()?.intent
                ?: throw AssertionError(MESSAGE_INTENT_NOT_FOUND)
            ...
            assertColorsValues(
                "$MESSAGE_WRONG_VALUES For x=80, y=90",
                expectedRgb,
                actualRgb,
                MARGIN_ERROR
            )
        ...
        testActivity {
            ...
            for (x in 0 until initialImageImmutable.width) {
                for (y in 0 until initialImageImmutable.height) {
                    ...
                    assertColorsValues(
                        "$MESSAGE_WRONG_VALUES For x=$x, y=$y",
                        expectedRgb1,
                        actualRgb1,
                        MARGIN_ERROR
                    )
